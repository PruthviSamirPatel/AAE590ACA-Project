...
%% ===================== EXTRA COAST TO MATCH TRUE LONGITUDE =====================
TargetAtMatch = propagate_orbit_J2(Earth, Target, tMatch);
nT = sqrt(mu / TargetAtMatch.a^3);
Ttarget = 2*pi/nT;
...
Ngrid = 200;
dtGrid = linspace(dtMin, dtMax, Ngrid);
phaseGrid = zeros(size(dtGrid));

for i = 1:Ngrid
    phaseGrid(i) = true_longitude_phase_error(dtGrid(i), tMatch, Earth, InterAchieved, Target);
end
...
for i = 1:Ngrid-1
    ...
    % Real zero crossing should change sign smoothly.
    % Reject jumps near the +/- pi wrap discontinuity.
    signChange = phaseGrid(i)*phaseGrid(i+1) < 0;
    smoothJump = abs(phaseGrid(i+1) - phaseGrid(i)) < deg2rad(90);

    if signChange && smoothJump
        idxBracket = i;
        break
    end
end

if isempty(idxBracket)

    warning('No smooth zero crossing found. Minimizing absolute true-longitude error instead.');

    costFun = @(dt) abs(true_longitude_phase_error(dt, tMatch, Earth, InterAchieved, Target));

    [dtExtra, minErr] = fminbnd(costFun, dtMin, dtMax);

    fprintf('Minimum achievable longitude error = %.9f deg\n', rad2deg(minErr));

else

    dtA = dtGrid(idxBracket);
    dtB = dtGrid(idxBracket+1);

    phaseFun = @(dt) true_longitude_phase_error(dt, tMatch, Earth, InterAchieved, Target);

    dtExtra = fzero(phaseFun, [dtA, dtB]);

end

tTotalCoast = tMatch + dtExtra;

InterCoasted  = propagate_orbit_J2(Earth, InterAchieved, tTotalCoast);
TargetCoasted = propagate_orbit_J2(Earth, Target,        tTotalCoast);

lambdaI = get_true_longitude_deg(InterCoasted);
lambdaT = get_true_longitude_deg(TargetCoasted);
lambdaErr = wrapTo180Local(lambdaI - lambdaT);

raanErr = wrapTo180Local(InterCoasted.raan - TargetCoasted.raan);

fprintf('\nExtra true-longitude coast = %.9f days\n', dtExtra/(24*3600));
fprintf('Total pre-transfer coast   = %.9f days\n', tTotalCoast/(24*3600));
fprintf('True longitude inter       = %.9f deg\n', lambdaI);
fprintf('True longitude target      = %.9f deg\n', lambdaT);
fprintf('True longitude error       = %.9f deg\n', lambdaErr);
fprintf('RAAN error after extra coast = %.9f deg\n', raanErr);
...
function phaseErr = true_longitude_phase_error(dtExtra, tMatch, Earth, InterAchieved, Target)

    tCoast = tMatch + dtExtra;

    InterNow  = propagate_orbit_J2(Earth, InterAchieved, tCoast);
    TargetNow = propagate_orbit_J2(Earth, Target,        tCoast);

    lambdaI = get_true_longitude_deg(InterNow);
    lambdaT = get_true_longitude_deg(TargetNow);

    phaseErr = deg2rad(wrapTo180Local(lambdaI - lambdaT));
end

function lambda_deg = get_true_longitude_deg(Orb)
    ...
    lambda_deg = mod(Orb.raan + Orb.argp + ta_deg, 360);
end
