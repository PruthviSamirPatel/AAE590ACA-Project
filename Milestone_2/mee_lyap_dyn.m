function Xdot = mee_lyap_dyn(~, X, xslowT_nd, K, P, mu, uMax, ...
    ...
    % Drift term (Keplerian)
    f0 = [0; 0; 0; 0; 0; sqrt(mu/p^3)*w^2];

    % MEE input matrix for RTN accelerations
    B = zeros(6,3);
    ...
    % Total commanded control
    u = uLyap + uBarrier;

    % Saturate commanded control only
    unorm = norm(u);
    if unorm > uMax
        u = uMax * u / unorm;
    end
    ...
    % Total dynamics = Kepler drift + control + J2 perturbation
    Xdot = f0 + B*(u + aJ2_rtn);
end